...
cfg = {
    'VGG12': [64, 64, 'M', 128, 128, 'M', 256, 256, 'M', 512, 'M', 512, 'M'],  # Hypothetical VGG12
}
...
def main():
    ...
    global test_rank
    test_rank = args.rank
    if specs['dataset'] == 'imagenet':
        out_size = 512
        for_cifar = False
        num_classes = 1000
        input_size = 224
        trn_dl, test_dl = get_imagenet_loaders(specs.get('use_valid_set', False))
        criterion = torch.nn.CrossEntropyLoss()
    ...
    if os.path.exists('./log/' + model_name + '_model.pth'):
        ...
        ist_model = ISTResNetModel(
            model=VGG(out_size=out_size, num_classes=num_classes).to(device),
